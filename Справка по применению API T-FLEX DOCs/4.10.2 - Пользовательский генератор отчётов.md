...
Пример реализации интерфейса ICustomizableReportGenerator для пользовательского генератора отчётовC#
...
Пример кода редактора шаблонов для пользовательского генератора отчётовC#
...
Пример шаблона для отчёта, который будет заполняться пользовательским генератором отчётовC#
